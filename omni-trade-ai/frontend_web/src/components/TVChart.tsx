...
function TVAdvancedChart({ symbol, timeframe, height, liveData }: Props) {
  ...
  return (
    <div style={{ position: "relative", height: height, width: "100%" }}>
      ...
      {/* AI Signal overlay — pinned top-left inside chart */}
      {liveData.price > 0 && (
        <div
          style={{
            position: "absolute",
        ...
        >
          ...
          <div
            style={{
              ...
              alignItems: "center",
              gap: 6,
              boxShadow: `0 0 14px ${signalColor}33`,
            }}
          >
            <div
              style={{
            ...
            />
            <span style={{ color: signalColor, fontWeight: 700, fontSize: 11, letterSpacing: "0.1em" }}>
              AI: {liveData.signal}
            </span>
            <span style={{ color: muted, fontSize: 9, fontWeight: 600 }}>
              {(liveData.confidence * 100).toFixed(0)}%
            </span>
          </div>
          ...
          {liveData.signal !== "HOLD" && liveData.entry_price > 0 && (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
                ...
                <span style={{ color: "#00E5FF", fontFamily: "monospace" }}>{fmt(liveData.entry_price)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
                <span style={{ color: muted }}>SL</span>
                <span style={{ color: "#FF9800", fontFamily: "monospace" }}>{fmt(liveData.stop_loss)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
                <span style={{ color: muted }}>TP</span>
                <span style={{ color: "#2196F3", fontFamily: "monospace" }}>{fmt(liveData.target_price)}</span>
              </div>
            </div>
          )}
        </div>
